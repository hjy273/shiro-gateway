package com.geekerstar.gateway.util.http;
...
import com.geekerstar.gateway.util.http.common.HttpResult;
import com.geekerstar.gateway.util.http.common.Utils;
import com.geekerstar.gateway.util.http.exception.HttpProcessException;
...
public class HttpClientUtil {
     ...
     * @throws HttpProcessException http处理异常
     */
    public static HttpResponse execute(HttpConfig config) throws HttpProcessException {
        create(config);//获取链接
        ...
        try {
            ...
            if (config.context() == null) {
                ...
                resp = config.client().execute(request, config.context());
            }
     ...
     * @return 返回Http处理request基类
     */
    private static HttpRequestBase getRequest(URI url, HttpMethods method) {
        HttpRequestBase request = null;
        switch (method.getCode()) {
            case 0:// HttpGet
                request = new HttpGet(url);
